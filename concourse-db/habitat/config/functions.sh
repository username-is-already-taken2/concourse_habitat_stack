...
work_mem=$(awk '/MemTotal/ {printf( "%.0f\n", (($2 / 1024 / 4) *3) / ({{cfg.postgresql.max_connections}} *3) )}' /proc/meminfo)MB
...
setup_replication_user_in_master() {
  ...
  psql -U {{cfg.superuser.name}}  -h {{svc.leader.sys.ip}} -p {{cfg.postgresql.port}} postgres >/dev/null 2>&1 << EOF
DO \$$
  BEGIN
...
setup_concourse_user() {
  echo 'Creating Concourse User'
  psql -U {{cfg.superuser.name}} --no-password -h localhost -p {{cfg.postgresql.port}} postgres >/dev/null 2>&1 << EOF
DO \$$
  BEGIN
  SET synchronous_commit = off;
  PERFORM * FROM pg_authid WHERE rolname = '{{cfg.concourse.db.username}}';
  IF FOUND THEN
    ALTER ROLE "{{cfg.concourse.db.username}}" WITH LOGIN PASSWORD '{{cfg.concourse.db.password}}';
  ELSE
    CREATE ROLE "{{cfg.concourse.db.username}}" WITH LOGIN PASSWORD '{{cfg.concourse.db.password}}';
  END IF;
END;
\$$
EOF
...
DO \$$
  BEGIN
    SET synchronous_commit = off;
    IF EXISTS (SELECT FROM pg_database WHERE datname = '{{cfg.concourse.db.name}}') THEN
      RAISE NOTICE 'PSQL - {{cfg.concourse.db.name}} database already exists';
    ELSE
      CREATE EXTENSION IF NOT EXISTS dblink;
      RAISE NOTICE 'PSQL - Creating "{{cfg.concourse.db.name}}" database with an owner of "{{cfg.concourse.db.username}}"';
      PERFORM dblink_connect('myconn','host=localhost port={{cfg.postgresql.port}} dbname=postgres user={{cfg.superuser.name}} password={{cfg.superuser.password}}');
      PERFORM dblink_exec('myconn', 'CREATE DATABASE "{{cfg.concourse.db.name}}" WITH OWNER "{{cfg.concourse.db.username}}";',false);
...
local_xlog_position() {
  psql -U {{cfg.superuser.name}} -h localhost -p {{cfg.postgresql.port}} postgres -t <<EOF | tr -d '[:space:]'
...
master_xlog_position() {
  psql -U {{cfg.superuser.name}} -h {{svc.leader.sys.ip}} -p {{cfg.postgresql.port}} postgres -t <<EOF | tr -d '[:space:]'
